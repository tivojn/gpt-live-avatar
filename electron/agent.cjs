...
function setupAgent(deps){
 const pending=new Map(),completed=new Map(),fileReferences=new Map();let recent=[];
 ...
 async function answer(sender,{id,history,turnId,character,onReceipt}={}){
  ...
  const agent=createAgentTools({config,request:latest,avatarCommand:(action,args,signal)=>command(sender,action,args,signal),referenceFile:fileReferences.get(sender.id),onReceipt:receipt=>{if(receipt.ok&&receipt.path&&['create_text_file','read_text_file','trash_file'].includes(receipt.tool))fileReferences.set(sender.id,receipt.path);if(typeof onReceipt==='function')onReceipt(receipt);},progress:value=>progress(sender,{id,...value})});
 ...
 return {answer,dispose(){for(const p of pending.values())p.finish(Error('App closed.'));pending.clear();completed.clear();fileReferences.clear();}};
}
