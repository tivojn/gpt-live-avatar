...
const {normalizeDelegate,usesCodexActions,codexConfig,reasoningEngine,actionEngine,runtimeConfig}=require('./delegate.cjs');
function setupAgent(deps){
 ...
 const runtimes=Object.fromEntries(['openclaw','hermes'].map(engine=>[engine,new (require('./acp-agent.cjs').AcpAgent)({engine,approve:approveRuntime})]));
 ...
 async function answer(sender,{id,history,turnId,character,onReceipt}={}){
  ...
  const agent=createAgentTools({config,request:latest,avatarCommand:(action,args,signal)=>command(sender,action,args,signal),referenceFile:fileReferences.get(sender.id),onReceipt:receipt=>{if(receipt.ok&&receipt.path&&['create_text_file','read_text_file','trash_file'].includes(receipt.tool))fileReferences.set(sender.id,receipt.path);if(actionEngine(config)==='basic'&&typeof onReceipt==='function')onReceipt(receipt);},progress:update});
  update({state:'thinking',tool:''});
  const engine=actionEngine(config);
  const work=engine!=='basic'
   ?(engine==='codex'?codex:runtimes[engine]).answer(sender.id,id,runtimeConfig(config),history,latest,speaker,agent,receipt=>onReceipt?.(receipt),update)
 ...
 handle('gla:agent:inventory',async()=>{const result={};for(const engine of ['openclaw','hermes'])result[engine]=await require('./runtime-agents.cjs').discoverAgents(engine,deps.getConfig());return {runtimes:result};});
