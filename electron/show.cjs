...
function directorInstructions(raw,{live=false}={}){
 ...
 const custom=c.pipeline.available?'Custom motions: when the script needs a movement nobody has installed, the Playwright lists it and you generate it with Meshy text-to-motion (about two to three minutes each, at most three per show), check that it faces the audience, retarget it and add it to every character.':`Custom motion generation is not configured on this ${process.platform==='darwin'?'Mac':'computer'} (missing ${c.pipeline.problems.join('; ')||'setup'}); the Playwright substitutes the closest installed motion instead.`;
