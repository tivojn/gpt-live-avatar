...
const NAMES={openclaw:'OpenClaw',hermes:'Hermes'};
function findRuntime(engine,override=''){
 ...
 const names=engine==='hermes'?['hermes','hermes-acp']:['openclaw'];
 const dirs=[path.join(home,'.openclaw/bin'),path.join(home,'.local/bin'),path.join(home,'.hermes/hermes-agent/venv/bin'),path.join(home,'.hermes/hermes-agent/.venv/bin'),'/opt/homebrew/bin','/usr/local/bin',...(process.env.PATH||'').split(path.delimiter).filter(Boolean)];
 const candidates=explicit?[explicit]:dirs.flatMap(dir=>names.map(name=>path.join(dir,name)));
 for(const file of candidates)try{if(path.isAbsolute(file)&&fs.statSync(file).isFile()){fs.accessSync(file,fs.constants.X_OK);return {file,args:path.basename(file)==='hermes-acp'?[]:['acp']};}}catch{}
...
class AcpClient{
 ...
 async start(){
  ...
  if(this.info.protocolVersion!==1)throw Error('This runtime uses an unsupported ACP version.');return this.info;
 }
