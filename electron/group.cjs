...
function setupGroup(deps){
 ...
 ipcMain.handle('gla:group:catalogue',guard(()=>{const settings=deps.getSettings();return {avatars:catalogue(),looks:settings.avatarLooks||{},defaults:settings.appearanceDefaults,voices:deps.voices,quality:settings.quality,hardware:settings.hardware,selected:deps.getConfig().avatar,hasKey:settings.hasKey,agentEnabled:settings.agentEnabled,agentFolder:settings.agentFolder,reasoning:settings.reasoningMode==='delegate'?settings.delegate.model:'gpt-5.6-luna'};}));
 ipcMain.handle('gla:group:reply',guard(async(event,request)=>{const r=conversationRequest(request,catalogue()),config=deps.getConfig();const choice=config.reasoningMode==='delegate'?config:normalizeDelegate(config,{reasoningMode:'delegate',delegateProvider:'openai',delegateAuth:'api_key',delegateModel:'gpt-5.6-luna'});if(config.agentEnabled&&request.humanRequest&&request.human?.enabled){const text=cleanText(request.humanRequest,6000);return deps.agentAnswer(event.sender,{id:r.id,turnId:request.turnId||r.id,history:[{role:'user',text}]});}return deps.backend.answer(event.sender.id,r.id,choice,r.history,r.instructions);}));
