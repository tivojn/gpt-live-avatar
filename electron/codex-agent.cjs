...
class CodexAgent{
 ...
 async answer(owner,id,config,history,request,character,tools,onReceipt=()=>{},progress=()=>{}){
  ...
  try{
   ...
   const started=await this.client.request('thread/start',{cwd:config.agentFolder,model,ephemeral:true,...codexPermissions(config.agentAccess),developerInstructions:instructions,dynamicTools});
