...
Right-click **Reasoning → provider** to choose reasoning. **Actions & Permissions → provider** contains that engine’s permissions and the option to use it for actions. The active action engine is checked and named explicitly. Missing installations are disabled.
