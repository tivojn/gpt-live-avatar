...
final class ModelResources: @unchecked Sendable {
    ...
    init(url: URL, maximumTextureSize: Int = 2048, textureBudget: Int = 256 * 1024 * 1024, cacheRoot: URL? = nil) throws {
        ...
        do {
            ...
            guard word(header, 0) == GLBConstants.magic, word(header, 4) == 2,
                  word(header, 8) == length, word(header, 16) == GLBConstants.jsonChunk else {
                throw AvatarModelError.notGLB
            }
            ...
            guard jsonLength > 0, jsonLength <= 16 * 1024 * 1024,
                  ...
                  word(binHeader, 4) == 0x004E4942 else { throw AvatarModelError.invalidJSON }
            ...
            guard binStart + binLength <= length,
                ...
                throw AvatarModelError.externalResources
            }
            var spans: [Span] = []
            for (index, view) in bufferViews.enumerated() {
                guard (view["buffer"] as? Int) == 0,
                    ...
                    throw AvatarModelError.invalidJSON
                }
            ...
            for (index, image) in sourceImages.enumerated() {
                guard let view = image["bufferView"] as? Int, spans.indices.contains(view),
                      image["uri"] == nil else { throw AvatarModelError.externalResources }
                let span = spans[view]
