...
fs.writeFileSync(path.join(app.getPath('userData'),'config.json'),JSON.stringify({avatar:'tia',reasoningMode:'managed',agentEnabled:false,quality:'friendly',bubbleMode:'off',avatarLooks:{tia:{}}}));
...
app.whenReady().then(async()=>{try{
 ...
 await js('await gla.showMenu({});');assert(menu.find(m=>m.label==='Reasoning'));await js("await gla.setSettings({reasoningMode:'managed'})");
