...
app.whenReady().then(async()=>{try{
 ...
 solo.webContents.send('gla:menu-action','agent');await wait(300);fs.writeFileSync(out+'/solo-result.png',(await solo.webContents.capturePage()).toPNG());await js("document.querySelector('#composerClose').click();await gla.group.open();");
