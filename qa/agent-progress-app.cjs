...
app.whenReady().then(async()=>{try{
 ...
 send(group,{id:'tia-next',character:'Tia',state:'thinking'});send(group,{id:'sarah-task',character:'Sarah',state:'error',error:'stale failure'});send(group,{id:'tia-next',character:'Tia',state:'update',text:'I’m creating the file you requested.'});await wait(200);assert(await run(group,"return gla_group.actors.get('sarah').bubbleText.textContent.includes('stale failure')") );assert(await run(group,"return gla_group.actors.get('tia').bubble.textContent.includes('creating the file')"));
 send(group,{id:'tia-next',character:'Tia',state:'cancelled'});await wait(2100);assert(await run(group,"return !gla_group.actors.get('tia').bubbleText.textContent"));
