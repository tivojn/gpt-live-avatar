...
app.whenReady().then(async()=>{try{
 ...
 await run(group,"await gla_group.actorMenuAction({slug:'sarah',action:'agent'});");assert.equal(await run(group,"return gla_group.actors.get('sarah').askInput.getAttribute('aria-label')"),'Request for Sarah');
 await run(group,`const a=gla_group.actors.get('sarah');a.askInput.value=${JSON.stringify('Read group-'+filename+' and tell me its contents, without changing it.')};a.askInput.dispatchEvent(new Event('input'));a.composer.querySelector('form').requestSubmit();`);
 await until(()=>run(group,"return gla_group.actors.get('sarah').activity?.active===false"),'Sarah context-menu action');const menuReply=await run(group,"return gla_group.actors.get('sarah').bubbleText.textContent");assert.match(menuReply,/original/);assert.match(menuReply,/verified/);
