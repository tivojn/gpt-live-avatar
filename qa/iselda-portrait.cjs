// Run with Electron. Uses a separate test profile and the local Iselda package.
const {app,BrowserWindow}=require('electron');
const fs=require('node:fs'),path=require('node:path'),assert=require('node:assert/strict');
const output=path.resolve(__dirname,'../build/qa-iselda-portrait');fs.mkdirSync(output,{recursive:true});
app.setPath('userData',path.join(output,'profile'));fs.mkdirSync(app.getPath('userData'),{recursive:true});
fs.writeFileSync(path.join(app.getPath('userData'),'config.json'),JSON.stringify({avatar:'iselda',quality:'best',bubbleMode:'off',avatarLooks:{iselda:{lighting:'studio',followCursor:'false'}}}));
