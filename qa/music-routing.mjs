...
finish({handled:true,ok:true,text:'Dancing along to Test song.'});await new Promise(r=>setTimeout(r,0));assert.equal(events.at(-1).spoken,'Dancing along to Test song.');assert.equal(peer.delegating,false);assert.equal(room.awaitingHuman,false);
