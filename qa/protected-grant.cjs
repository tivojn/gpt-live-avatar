...
app.whenReady().then(async()=>{const root=fs.mkdtempSync(path.join(os.tmpdir(),'gla-key-grant-'));let server;try{
 ...
 fs.mkdirSync(path.join(options.bundledRoot,'fixture'),{recursive:true});fs.copyFileSync(path.join(output,remote.file),path.join(options.bundledRoot,'fixture/base.gla'));
 const first=new AvatarAssets(options);assert(!first.installed('fixture'),'First install needs an online unlock');await Promise.all([first.unlockInstalled(),first.ensureKeys()]);assert(first.bundled('fixture')&&first.installed('fixture'),'Bundled starter unlocks without a model download');assert.equal(authorizations,1,'Concurrent requests share one authorization');assert(!fs.readFileSync(first.keyFile).includes(Buffer.from(Object.values(keys)[0])),'Content key is never stored as plaintext');
