...
server.listen(0,'127.0.0.1',()=>{
 ...
 app.whenReady().then(async()=>{try{
  ...
  {const voiceMenu=template.find(x=>x.label==='Voice').submenu;assert.match(voiceMenu[0].label,/^Gemini 3\.8 Live voices/);assert.equal(voiceMenu.filter(x=>x.submenu).length,30);assert(voiceMenu.some(x=>x.label==='Kore · female · firm ✓'),'her current Gemini voice is ticked');assert(voiceMenu.some(x=>x.label==='Puck · male · upbeat'),'Google’s published gender beside each voice');assert(!voiceMenu.some(x=>/Marin|Gleam/.test(x.label)),'no GPT-Live-1 voices while Gemini is the system');
