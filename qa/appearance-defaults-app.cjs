...
async function ready(w,slug){await until(()=>run(w,`const s=await gla.getSettings();return s.avatar.slug===${JSON.stringify(slug)}&&window.gla_avatar?.resources.ready&&gla_avatar.characterId===${JSON.stringify(slug)}&&gla_avatar.appearance?.indexURL===s.avatar.appearanceURL;`),slug+' ready');await wait(350);}
