...
(async()=>{const root=fs.mkdtempSync(path.join(os.tmpdir(),'gla-protected-'));let server;try{
 ...
 // Public installer routes: no token, at most one R2 read each; the catalogue and parts stay closed.
 const installerURL='https://downloads.example/releases/gpt-live-avatar-0.2.22-arm64.dmg';
 const latestDoc={version:'0.2.22',title:'T',notes:'<b>notes</b>',publishedAt:'2026-09-18T00:00:00.000Z',arch:'arm64',url:installerURL,sha256:'a'.repeat(64),bytes:5,installers:{arm64:{url:installerURL,sha256:'a'.repeat(64),bytes:5}}};
 const store=new Map([['releases/latest.json',Buffer.from(JSON.stringify(latestDoc))],['releases/gpt-live-avatar-0.2.22-arm64.dmg',Buffer.from('dmg!!')]]);
 let publicReads=0;const publicEnv={DOWNLOAD_TOKEN:'test-token',AVATAR_ASSETS:{get:async name=>{publicReads++;const body=store.get(name);return body?{size:body.length,body}:null;}}};
 const pub=(p,init)=>worker.fetch(new Request('https://downloads.example/'+p,init),publicEnv);
 let r=await pub('releases/latest.json');assert.equal(r.status,200);assert.equal(r.headers.get('Content-Type'),'application/json');assert.equal(r.headers.get('Cache-Control'),'public, max-age=300');assert.deepEqual(await r.json(),latestDoc);assert.equal(publicReads,1);
 r=await pub('releases/gpt-live-avatar-0.2.22-arm64.dmg');assert.equal(r.status,200);assert.equal(r.headers.get('Content-Type'),'application/x-apple-diskimage');assert.equal(r.headers.get('Content-Disposition'),'attachment; filename="GPT-Live Avatar-0.2.22-arm64.dmg"');assert.equal(r.headers.get('Content-Length'),'5');assert.equal(await r.text(),'dmg!!');assert.equal(publicReads,2);
 r=await pub('releases/gpt-live-avatar-0.2.22-arm64.dmg',{method:'HEAD'});assert.equal(r.status,200);assert.equal(r.headers.get('Content-Length'),'5');assert.equal(publicReads,3);
 r=await pub('releases/gpt-live-avatar-0.2.22-arm64.dmg',{headers:{Range:'bytes=0-1'}});assert.equal(r.status,416);assert.equal(publicReads,3,'Rejected ranges cost no read');
 r=await pub('releases/');assert.equal(r.status,200);assert.equal(r.headers.get('Content-Type'),'text/html; charset=utf-8');const html=await r.text();assert(html.includes('href="'+installerURL+'"'),'The page links the same-origin installer');assert(html.includes('&lt;b&gt;notes&lt;/b&gt;')&&!html.includes('<b>notes</b>'),'Notes are escaped');assert.equal(publicReads,4);
 store.set('releases/latest.json',Buffer.from(JSON.stringify({...latestDoc,url:'https://evil.example/x.dmg',installers:{arm64:{...latestDoc.installers.arm64,url:'https://evil.example/x.dmg'}}})));
 assert(!(await (await pub('releases/')).text()).includes('evil.example'),'Off-origin installer links are never rendered');store.set('releases/latest.json',Buffer.from(JSON.stringify(latestDoc)));publicReads=4;
 for(const p of ['releases/gpt-live-avatar-0.2.23-arm64.dmg','releases/index.json','releases/GPT-Live.Avatar-0.2.22-arm64.dmg','releases/latest.json?x=1','releases/latest.json.bak','releases/gpt-live-avatar-0.2.22-arm64.dmg/'])assert.equal((await pub(p)).status,404,p);
 ...
 storageStatus=503;await assert.rejects(cross.get('fixture-base.gla.p000'));assert.equal(requests,2,'Transient failures never cause hidden R2 retries');await assert.rejects(cross.get('../secret'));assert.equal(requests,2,'Invalid paths never reach storage');for(const bad of ['releases/../secret','releases/./x','/releases/latest.json','releases//latest.json','releases/','Releases/latest.json'])await assert.rejects(cross.get(bad));assert.equal(requests,2,'Dot, empty and uppercase segments never reach storage');storageStatus=200;assert.equal((await cross.get('releases/latest.json')).size,4);assert.equal(new URL(lastRequest.url).pathname,'/test-private/releases/latest.json','Public release objects are read from the same bucket');storageStatus=404;await assert.rejects(cross.get('releases/latest.json'),error=>error.status===404,'Missing objects report their status');
