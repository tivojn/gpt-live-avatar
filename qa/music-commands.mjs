...
const starts={
 'sing along':'dance_along',
 'Can you sing a long to this song?':'dance_along',
 'Sarah, can you please sing along with this song?':'dance_along',
 'Please start singing along with the music.':'dance_along',
 'sing and dance along':'dance_along',
 'sing along and dance':'dance_along',
 ...
 'Stop singing':'stop_dancing',
 'Please stop singing and dancing':'stop_dancing',
 'stop dancing along':'stop_dancing',
 '结束':'none',
 '跟着这首歌唱歌':'dance_along',
 '请跟着音乐跳舞':'dance_along',
 '停止跟唱':'stop_dancing',
...
assert.equal((await a).ok,true);assert.equal(calls,1);assert.equal(before,1);assert.equal(outcomes.length,1);assert.match((await a).text,/I don’t sing along, but I’ll dance along to Test song/,'a sing-along request is honoured as a dance, and says so');
...
let release;const wait=new Promise(r=>release=r),executed=[],stale=new MusicCommandRouter({execute:async(action,_args,cancelled)=>{executed.push(action);if(action==='dance_along')await wait;return {ok:!cancelled()};}});
