...
function render(info) {
  ...
  const states = { idle: ['Check for a newer version', 'Updates come from the official GPT-Live Avatar release service.'],
    ...
    available: ['Version ' + u.version + ' is available', u.problem || (u.installable ? 'Download it here. It is checked against the published checksum, its signature and Apple’s notarization before anything is installed. Your settings stay in place.' : info.platform === 'win32' ? (u.downloadURL ? 'Installing from inside the app is not available on Windows yet. Download the installer here and run it; your settings stay in place.' : 'Updating from inside the app is not available on Windows yet. Get the new installer from the release page and run it; your settings stay in place.') : u.downloadURL ? 'Download the installer, then replace the app in Applications. Your saved settings remain in place.' : 'An installer for this Mac is not attached. Check the release page for available downloads.')],
