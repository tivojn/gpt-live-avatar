...
export function installShow({api,voice,actors,catalogue,hooks}){
 ...
 function renderScript(){
  const body=$('#showScriptBody');body.replaceChildren();if(!script){$('#showScript').hidden=true;return;}
  $('#showScript').hidden=false;const names=new Map(script.cast.map(c=>[c.slug,`${c.role} (${c.name})`]));if(script.userRole)names.set('user',`${script.userRole.role} (${script.userRole.name})`);
  ...
  script.scenes.forEach((scene,si)=>{const h=document.createElement('h3');h.textContent=`Scene ${si+1}: ${scene.title}`;body.append(h);if(scene.setting){const s=document.createElement('p');s.className='stage';s.textContent=scene.setting;body.append(s);}
   scene.lines.forEach((line,li)=>{const p=document.createElement('p');p.dataset.cue=si+':'+li;if(line.speaker==='user')p.className='user-line';const b=document.createElement('b');b.textContent=(names.get(line.speaker)||line.speaker)+': ';p.append(b,document.createTextNode(line.text));if(line.motion||line.note){const em=document.createElement('span');em.className='stage';em.textContent=' ['+[line.note,line.motion].filter(Boolean).join(' · ')+']';p.append(em);}body.append(p);});});
 }
 // ------------------------------------------------------------ Performance
 ...
 function stage(){
  ...
  return {
   ...
   speak:async(cue,slug)=>{const a=actorOf(slug);if(!a)throw Error('That character left the stage.');a.el.classList.remove('standby');const heard=await voice.speak(cue.text,hooks.voiceFor(slug),text=>hooks.setBubble(a,text));await pause(350);return heard;},
