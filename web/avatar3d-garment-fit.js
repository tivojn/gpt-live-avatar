import { bindSarahBodyBrief } from '/avatar3d-body-brief.js';
import {fitMingMeiDress} from '/avatar3d-mingmei-fit.js';
...
export function garmentEase(character,name,x,y,z){
 ...
 return .006*(1-smooth(y,1.07,1.14))+.022*(1-smooth(Math.abs(x),.035,.12))*smooth(y,.865,.91)*(1-smooth(y,1.01,1.12))*smooth(z,.015,.06);
}
export function fitGarment(character,node){
 if(character==='sarah')bindSarahBodyBrief(node);
 if(character==='ming-mei'&&(node.userData.sourceName||node.name)==='Dress01')return fitMingMeiDress(node);
