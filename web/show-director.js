...
export class DirectorVoice {
 ...
 emit(type,detail,extra){this.callbacks[type]?.(detail,extra);}
 async start({voice,context,floorOpen=true}){
  this.stop();const generation=++this.generation,current=()=>generation===this.generation;this.running=true;this.floorOpen=floorOpen;this.floorReason='';
  try{
   ...
   const client=this.client=new LiveClient({createSession:sdp=>this.api.directorLive({sdp,voice,floorOpen,context:this.contextFor?.()||context})});
   // Cancel only the Director's own delegation; the Playwright's script must survive a hang-up.
   this.delegate=new DelegateClient(client,{answer:({id,history})=>this.api.director({id,history,context:this.contextFor?.()||context}),cancel:id=>this.api.cancel({scope:'director',id})},message=>this.emit('status',message));
 ...
 closeFloor(reason='The show is being prepared and performed.'){
  if(!this.client||!this.floorOpen&&this.floorReason===reason)return;this.floorOpen=false;this.floorReason=reason;this.output?.setActive(false);
 ...
 openFloor(prompt=''){
  if(!this.client)return;this.floorOpen=true;this.floorReason='';this.output?.setActive(true);
