...
export class GroupVoice {
 ...
 async speak(text,voice,onText,delivery=''){
  ...
  return new Promise((resolve,reject)=>{
   ...
   const attach=stream=>{if(generation!==this.generation||done)return;this.context=new AudioContext();this.output=new SpeechOutput(this.context,{onError:message=>this.onWarning?.(message)});this.output.attach(stream);void this.context.resume().catch(()=>complete(Error('Click Start to allow playback.')));};
