...
async function start(){
 ...
 if(running||loading||actors.size<2)return;const topic=$('#topic').value.trim()||$('#topic').placeholder.trim();if(!topic){status('Add a topic first.',true);return;}
...
$('#start').onclick=start;$('#stop').onclick=()=>stop();$('#close').onclick=async()=>{stop();try{await Promise.race([show?.settle?.(),new Promise(r=>setTimeout(r,20000))]);}catch{}void api.close();};$('#headClose').onclick=()=>$('#close').onclick();$('#reset').onclick=arrange;$('#compact').onclick=()=>compact(!$('.panel').classList.contains('compact'));
$('#join').onchange=()=>{$('#nameLabel').hidden=!$('#join').checked;show?.refresh();};
$('#format').onchange=()=>{const isShow=showMode();$('.panel').classList.toggle('show-mode',isShow);$('#joinLabel').textContent=isShow?'I’ll act a role':'Join as yourself';if(!isShow)show?.leave();else if(running)stop('Switched to Avatar Show.');show?.refresh();$('#topic').placeholder=({show:'A short comedy about a lost crown, with a twist ending.',chat:'Plan a cheerful day out together, each suggesting something different.',story:'A mysterious invitation arrives from a floating city. We decide what happens next.',debate:'Is the best holiday carefully planned or completely spontaneous?',choices:'Would you rather have a tiny dragon companion or a door to anywhere? Explore our choices.'})[$('#format').value];};
