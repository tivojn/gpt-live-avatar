...
async function main(){
 ...
 const index=JSON.parse(previous.payload),starterAvatar=loadDefaultAvatar(repo);if(!index.avatars[starterAvatar.slug])throw Error('Default avatar is absent from the signed catalogue: '+starterAvatar.name);index.release=revision;
 ...
 const starterRoot=path.join(out,'starter',starterAvatar.slug);fs.mkdirSync(starterRoot,{recursive:true});
 const starter=path.join(starterRoot,'motions.gla');fs.rmSync(starter,{force:true});fs.linkSync(path.join(out,index.avatars[starterAvatar.slug].motionUpdate.package.file),starter);
