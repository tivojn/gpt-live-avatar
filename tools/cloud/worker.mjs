...
// releases/ is the only unauthenticated surface: the Apple-signed installer,
// its metadata (latest.json) and a small landing page. The signed catalogue
// (index.json), key wrapping and encrypted parts stay token-authenticated.
import { createR2Reader } from './r2-reader.mjs';
export const RELEASE_PREFIX='releases/';
export const INSTALLER_NAME=/^gpt-live-avatar-((?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*))-(arm64|x64)\.dmg$/;
const escapeHTML=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[c]);
function releasePage(doc,origin){
 const version=typeof doc.version==='string'&&/^\d+\.\d+\.\d+$/.test(doc.version)?doc.version:'';
 const installers=[];
 for(const [arch,label] of [['arm64','Apple silicon'],['x64','Intel']]){
  const entry=doc.installers?.[arch]||(doc.arch===arch?doc:null);
  const name='gpt-live-avatar-'+version+'-'+arch+'.dmg';
  // Only same-origin installer names are linked, whatever the metadata says.
  if(version&&entry&&entry.url===origin+'/'+RELEASE_PREFIX+name&&/^[a-f0-9]{64}$/.test(entry.sha256||''))installers.push({label,url:entry.url,sha256:entry.sha256,bytes:Number.isSafeInteger(entry.bytes)?entry.bytes:null});
 }
...
<main><h1>${title}</h1><p class="meta">Version ${escapeHTML(version)}${date?' · '+date:''} · macOS 14 or newer</p>
...
<p>Open the DMG, copy GPT-Live Avatar to Applications and launch it. Verify with <code>shasum -a 256</code> if you wish. Existing installs: right-click any avatar → Check for Updates…</p>
...
export function createWorker(catalogue,objects){
 ...
 async function serveRelease(request,env,url,name,fail){
  ...
  if(rest!==''&&rest!=='latest.json'&&!installer)return fail(404,'Not found.');
  if(installer&&request.headers.has('Range'))return fail(416,'Retry this download in full.');
  const storage=env.AVATAR_ASSETS||createR2Reader(env);if(!storage)return fail(503,'Downloads are not activated.');
  let object;
  try{object=await storage.get(installer?name:RELEASE_PREFIX+'latest.json');}
  ...
  if(!object)return fail(404,installer?'Not found.':'No release has been published yet.');
  const cancel=async()=>{try{await object.body?.cancel?.();}catch{}};
  if(installer){
   const headers={'Content-Type':'application/x-apple-diskimage','Content-Length':String(object.size),'Content-Disposition':'attachment; filename="GPT-Live Avatar-'+installer[1]+'-'+installer[2]+'.dmg"','Cache-Control':'public, max-age=86400, immutable','X-Content-Type-Options':'nosniff'};
