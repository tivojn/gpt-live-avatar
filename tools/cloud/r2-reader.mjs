...
export function createR2Reader(env, fetcher = fetch) {
  ...
  return { async get(name) {
    // Flat encrypted parts, plus the public installer objects under releases/. No dot segments.
    if (name.split('/').some(segment => !/^[-a-z0-9.]+$/.test(segment) || /^\.+$/.test(segment))) throw Error('Invalid object name');
    ...
    if (response.status !== 200) { await response.body?.cancel(); throw Object.assign(Error('Storage unavailable: HTTP '+response.status), { status: response.status }); }
