"""Bake a Meshy FBX through Tia's original Auto-Rig Pro controls.
...
if args.preset:
    ...
    for target,source in mapping:
        if source.startswith('Spine'):
            source_end={'Spine1':'Spine2','Spine2':'Spine3','Spine3':'Neck'}[source]
            tn=int(target.split('_')[2].split('.')[0]);target_end='c_spine_%02d.x'%(tn+1) if tn<5 else 'c_subneck_1.x'
            align[target]=body_frame(donor_rest['L_Collar'].translation-donor_rest['R_Collar'].translation,donor_rest[source_end].translation-donor_rest[source].translation)@body_frame(control_rest['c_shoulder.l'].translation-control_rest['c_shoulder.r'].translation,control_rest[target_end].translation-control_rest[target].translation).inverted()
